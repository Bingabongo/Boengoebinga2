...
# Create list of all possible feature set combinations
# Possible columns: PM, H2S, wind, SO2, CO, ozone, NO_X_Y
all_features = [PM, H2S, wind, SO2, CO, ozone, NO_X_Y]
all_features_str = ['PM25', 'H2S', 'SONIC', 'SO2', 'CO', 'OZONE', 'NO']
all_combinations = []
for i in range(1, len(all_features)+1):
    combination = itertools.combinations(all_features, i)
    x = 0
    for j in combination:
        x += 1
        flat_list = [item for sublist in j for item in sublist]
        all_combinations.append(flat_list)
    print(f'{x} combinations')
print(f'{len(all_combinations)} possible combinations to test')

# Iterate over feature sets
i = 0
# Iterate 10 times over random variables within a given range
for iteration in range(10):
    print(f'big iteration {iteration}')

    pred_hour = random.randrange(1, 24, 2)
    threshold_hour = random.randrange(1, 15, 1)
    look_back_hr = random.randrange(1, 24, 1)

    # Iterate over all possible combinations
    for i, sensorset in enumerate(all_combinations):
        print(f'iteration combination {i + 1}/{len(all_combinations)}')
        wanted_cols = ['DateTime'] + list(sensorset)
        df_sensor = df_sensor_raw[wanted_cols]

        # Find short names of features
        features = []
        for feature in all_features_str:
            if any(feature in s for s in sensorset):
                if feature == "SONIC":
                    feature = "wind"
                if feature == "NO":
                    feature = "NO_X_Y"
        ...
        with open('tests_1.csv', 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                model, features, smell_thr_per_hour, smell_predict_hrs, look_back_hrs, train_size,
