...
wanted_cols = ['DateTime'] + PM + H2S + wind + CO
features = ['PM', 'H2S', 'wind', 'CO']
...
# Iterate over all possible combinations within a given range for the parameters
i = 0
for pred_hour in range(1, 25, 2):
    for threshold_hour in range(1, 14, 2):
        for look_back_hr in range(1, 24, 1):

            i = i + 1
            print(f'iteration: {i}/{2*7*3*14}')
            print(features, pred_hour, threshold_hour, look_back_hr)
            ...
            with open('tests_2.csv', 'a', newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
