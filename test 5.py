...
# Feature importance. Preset which features to test from
good_features = PM + wind + SO2 + CO + ozone + NO_X_Y
print(f'{len(good_features)} features total')
print(good_features)

# Preset base F1 score. Stops if first iteration scores worse
base_f1_score = 0.675
max_f1 = base_f1_score
prev_max_f1 = base_f1_score

# Manually remove features beforehand. Keep empty to start over
remove_features = ['3.feed_3.SIGTHETA_DEG', '3.feed_3506.OZONE', '3.feed_11067.NOX_PPB..3.feed_43.NOX_PPB',
                   '3.feed_29.PM25_UG_M3', '3.feed_26.SONICWD_DEG']
for i in remove_features:
    good_features.remove(i)

# If new feature set is better than old feature set, keep going else stop
while max_f1 >= prev_max_f1:

    # New feature set is best from previous iteration
    prev_max_f1 = max_f1
    f1_scores = {}

    # Iterate over all remaining features. Removing one per time from the remaining feature set
    for i, feature in enumerate(good_features):
        feature_set = ['DateTime'] + good_features
        print(f'test iteration {i}')
        feature_set.remove(feature)

        print(f'{len(good_features)} features remaining')
        print(f'iteration {i + 1}/{len(good_features)}')
        ...
        f1_scores[feature] = round(f1_2, 3)

        # Save model and scores to csv file
        with open('tests_5.csv', 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                model, remove_features, smell_thr_per_hour, smell_predict_hrs, look_back_hrs, train_size,
                test_size, accuracy, precision, recall, f1, f1_2
            ])

    # Determine which feature scores highest F1 if removed
    max_f1, remove_f1_feature = max(zip(f1_scores.values(), f1_scores.keys()))

    # Remove feature from all features
    remove_features.append(remove_f1_feature)
    good_features.remove(remove_f1_feature)

    print("")
    print(f'F1: {round(max_f1, 4)} - removing {remove_f1_feature}')
    print("")
